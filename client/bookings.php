...
<div class="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-2 mb-4">
    <div>
        <h4 class="fw-bold mb-1" style="color:#0f172a;">Bookings & Pass Holders</h4>
        <p class="text-muted small mb-0">View attendee registrations, ticket counts, and manage pass statuses.</p>
    </div>
    ...
    <form method="GET" action="/client/bookings" class="row g-2 align-items-center">
        <div class="col-12 col-md-4">
            <div class="input-group input-group-sm">
                ...
                <input type="text" name="search" class="form-control" placeholder="Search by name, phone, booking #..." value="<?= e($search) ?>">
            </div>
        </div>
        ...
        <table class="table table-hover align-middle mb-0">
            ...
            <tbody>
                <?php if (empty($bookings)): ?>
                    <tr><td colspan="7" class="text-center py-4 text-muted">No bookings found matching current filters.</td></tr>
                <?php else: ?>
                    <?php foreach ($bookings as $b): ?>
                        <tr>
                            ...
                            <td class="small text-secondary">
                                <div><?= e($b['email']) ?></div>
                                <div class="font-monospace text-muted" style="font-size:11.5px;"><?= e($b['phone']) ?></div>
                            </td>
                            <td>
                            ...
                            <td class="text-end">
                                <div class="d-inline-flex gap-1">
                                    <a href="/verify/<?= e($b['qr_token']) ?>" target="_blank" class="btn btn-sm btn-outline-secondary py-1 px-2.5 text-xs shadow-xs" title="View Digital Pass">
                                    ...
                                    <?php if ($b['status'] === 'pending'): ?>
                                        <form method="POST" class="d-inline" onsubmit="return confirm('Confirm this booking?')">
                                            ...
                                            <button type="submit" class="btn btn-sm btn-success py-1 px-2 text-xs shadow-xs">Approve</button>
                                        </form>
                                    <?php endif; ?>
                                    <?php if ($b['status'] !== 'cancelled'): ?>
                                        <form method="POST" class="d-inline" onsubmit="return confirm('Cancel this pass?')">
                                            ...
                                            <button type="submit" class="btn btn-sm btn-outline-danger py-1 px-2 text-xs shadow-xs">Cancel</button>
                                        </form>
                                    <?php endif; ?>
                                </div>
                            </td>
